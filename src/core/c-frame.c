...
***********************************************************************/
{
	...
	for (n = 0; n < SERIES_TAIL(Bind_Table); n++) {
		if (binds[n]) {
			Debug_Fmt((const REBYTE *)"Bind table fault: %3d to %3d (%s)", n, binds[n], Get_Sym_Name(n));
		}
	}
}
...
***********************************************************************/
{
	...
	for (n = 0; n < tail; n++, values++, words++) {
		if (IS_END(words) || IS_END(values)) {
			Debug_Fmt((const REBYTE *)"** Early %s end at index: %d", IS_END(words) ? "words" : "values", n);
		}
	}

	if (NOT_END(words) || NOT_END(values))
		Debug_Fmt((const REBYTE *)"** Missing %s end at index: %d type: %d", NOT_END(words) ? "words" : "values", n, VAL_TYPE(words));
}
...
***********************************************************************/
{
	...
	Set_Root_Series(TASK_BUF_WORDS, Make_Block(100), (REBYTE *)"word cache"); // just holds words, no GC
}
